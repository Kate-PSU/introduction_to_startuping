...
function renderSignals(){
  ...
  $("#checkSignals").onclick=()=>{const right=[0,2,4],ok=right.every(x=>state.selected.has(x));document.querySelectorAll(".choice").forEach((b,i)=>right.includes(i)?b.classList.add("correct"):state.selected.has(i)&&b.classList.add("wrong"));$("#missionFeedback").innerHTML=feedback(ok?"Точно. Стартап связан с инновациями, поиском масштабируемой модели и стремлением к росту.":"Проверьте выбор. В лекции ключевыми названы инновации, поиск масштабируемой модели, рост и изменения.",ok);ok?celebrate():fail();const btn=$("#checkSignals");btn.textContent="Продолжить";btn.disabled=false;btn.onclick=()=>{state.selected.clear();next(ok?20:10)}};
}
